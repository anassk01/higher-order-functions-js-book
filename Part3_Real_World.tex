...
\toprule
\hoftablehead{Pattern} & \hoftablehead{Use When} \\
\midrule
...
\code{useMemo} memoizes the \textbf{result} of calling the function.
\code{useCallback} memoizes the \textbf{function itself}.
These are equivalent: \code{useCallback(fn, deps)} and \code{useMemo(() => fn, deps)}.
\end{definition}
...
The signature \code{store => next => action => result} creates a triple-nested HOF that can intercept, modify, or delay actions.
\end{definition}
...
\toprule
\hoftablehead{Domain} & \hoftablehead{Patterns Learned} \\
\midrule
