...
\begin{danger}
\textbf{Always Provide Initial Value!}
...
\begin{note}
Each \func{map} and \func{filter} creates a new array:
...
\textbf{For small arrays (< 10,000 items):} Don't worry. The clarity is worth it.

\textbf{For large arrays:} We'll cover optimization in Chapter 12 (transducers, lazy evaluation).
\end{note}
...
\begin{warning}
\textbf{Empty Array:} \func{every} returns \code{true}
...
\begin{enumerate}
    \item \textbf{Get all active users with normalized emails (lowercase)}

    \item \textbf{Calculate total salary expense for active employees}

    \item \textbf{Group active users by department}

    \item \textbf{Find the highest paid active employee}

    \item \textbf{Check if all Engineering employees are active}

    \item \textbf{Get list of departments that have at least one active employee}

    \item \textbf{Calculate average salary per department (active employees only)}

    \item \textbf{Create a summary string for each active user}
\end{enumerate}
